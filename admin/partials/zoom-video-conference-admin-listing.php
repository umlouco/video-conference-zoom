...
<?php
	...
	if($query_string == "page=list_meetings"):
		...
		if(get_option('zoom_api_key') && get_option('zoom_api_secret')):
			foreach($users_arr as $userid) {
				$data = $api->listMeetingsCustom($userid);
			...
			<div class="wrap"><h1><?php _e('Meeting List<a href="?page=add_meeting" class="page-title-action">Add Meeting</a>', 'zoom-video-conference'); ?></h1>
				...
				<table class="wp-list-table widefat fixed striped posts">
					<thead>
						<tr>
							<th width="8%" scope="col" id="meeting_id" class="manage-column column-primary"><span><?php _e('Meeting ID', 'zoom-video-conference'); ?></span></th>
							<th width="20%" scope="col" id="topic" class="manage-column column-primary"><span><?php _e('Topic', 'zoom-video-conference'); ?></span></th>
							<th width="6%" scope="col" id="status" class="manage-column column-primary"><span><?php _e('Status', 'zoom-video-conference'); ?></span></th>
							<th width="12%" scope="col" id="type" class="manage-column column-primary"><span><?php _e('Type', 'zoom-video-conference'); ?></span></th>
							<th width="14%" scope="col" id="start_time" class="manage-column column-primary"><span><?php _e('Start Time', 'zoom-video-conference'); ?></span></th>
							<th width="18%" scope="col" id="join_url" class="manage-column column-primary"><span><?php _e('Host ID', 'zoom-video-conference'); ?></span></th>
							<th width="14%" scope="col" id="created_on" class="manage-column column-primary"><span><?php _e('Created On', 'zoom-video-conference'); ?></span></th>
							<?php if($current_user->roles[0] != 'instructor'): ?>
					...
					<tbody id="the-list">	
						<?php if( count($total_meetings) > 0 ): ?>
							<?php foreach($total_meetings as $result): ?>
								<tr>
									...
									<td><a href="admin.php?page=list_meetings&edit=<?php echo $result['id']; ?>&host_id=<?php echo $result['host_id']; ?>"><?php echo $result['topic']; ?></a></td>
									<td><?php $type = $result['status']; 
										switch($type) {
											case 0;
											echo '<img src="'.plugins_url().'/zoom-api/admin/img/2.png" style="width:14px;">';
											break;
											case 1;
											echo '<img src="'.plugins_url().'/zoom-api/admin/img/3.png" style="width:14px;">';
											break;
											case 2;
											echo '<img src="'.plugins_url().'/zoom-api/admin/img/1.png" style="width:14px;">';
											break;
											default;
											break;
										}
										?></td>
										<td><?php $type = $result['type']; 
											switch($type) {
												case 1;
												echo 'Instant Meeting';
												break;
												case 2;
												echo 'Scheduled Meeting';
												break;
												default;
												break;
											}
											...
											<!-- <td><a href="<?php echo $result['join_url']; ?>" target="_blank">Start Meeting</a></td> -->
											<td><?php echo $result['host_id']; ?></td>
											<td><?php echo $result['created_at']; ?></td>
											<?php if($current_user->roles[0] != 'instructor'): ?>
												<td><a href="#" onclick="confirm_delete(<?php echo $result['id']; ?>, '<?php echo $result['host_id']; ?>')"><?php _e('Trash', 'zoom-video-conference'); ?></a>/ <a href="<?php echo $result['join_url']; ?>" target="_blank"><?php _e('Start Meeting', 'zoom-video-conference'); ?></a></td>
											<?php endif; ?>
										</tr>
									<?php endforeach; ?>
								<?php else: ?>
						...
						<?php else: ?>
							<?php
							/**BEGIN CODING FOR THE EDIT SECTION **/
	$url = $_SERVER['REQUEST_URI']; //GETTING THE URL
	...
	<div class="wrap">
		<h1>Edit <?php echo $results['topic']; ?></h1>
		<div id="poststuff">
			<div id="post-body" class="metabox-holder columns-2">
				<form action="?page=list_meetings&edit=<?php echo $results['id']; ?>&host_id=<?php echo $host_id; ?>" method="POST">
					<div id="post-body-content" style="position: relative;">
						...
						<div id="titlediv">
							<div id="titlewrap">
								<input type="text" placeholder="Enter Meeting Topic" required name="meetingTopic" size="30" value="<?php echo ($results['topic']) ? $results['topic'] : ''; ?>" id="title" class="meetingTopic" >
							</div>
						</div>
					</div>
					<div id="postbox-container-1" class="postbox-container">
						...
						?>
						<div id="submitdiv" class="postbox ">
							<h2 class="hndle ui-sortable-handle"><span><?php _e('Link to Online Course', 'zoom-video-conference'); ?></span></h2>
							<div class="inside">
								<div id="major-publishing-actions">
									<select style="width: 214px;" name="post_id_course" class="zoom_api_courses">
										<option custom_title="" value=""><?php _e('Select a Course', 'zoom-video-conference'); ?></option>
										<?php while($query->have_posts()): $query->the_post(); ?>
											<option value="<?php the_ID(); ?>" data-title="<?php the_title(); ?>"><?php the_title(); ?></option>
										<?php endwhile; ?>
									</select><button id="add_linkto_course"><?php _e('Add', 'zoom-video-conference'); ?></button><br>
									<small><?php _e('You can only link one video to one course at a time.', 'zoom-video-conference'); ?></small>
								</div>
							</div>
						</div>

						<div id="submitdiv" class="postbox ">
							<h2 class="hndle ui-sortable-handle"><span><?php _e('Courses Linked', 'zoom-video-conference'); ?></span></h2>
							<div class="inside">
								<div id="major-publishing-actions" class="linked_result_diplay">
									<?php while($query->have_posts()): $query->the_post(); ?>
										<?php 
										$get_zoom_meta1 = get_post_meta( get_the_id(), 'vibe_zoom_video', true );
										$decoded_result1 = json_decode($get_zoom_meta1);
										if(!empty($decoded_result1->post_id)):
											?>
										<p id="title_<?php echo get_the_id(); ?>"><?php echo ($decoded_result1->meeting_id == $_GET['edit']) ? the_title().' <a onclick="delete_linked_course('.get_the_id().');" href="#">Delete</a>' :''; ?></p>
									<?php endif; ?>
								<?php endwhile; ?>
					...
					<div id="date-settings" class="postbox ">
						...
						<div class="inside">
							<table class="zoom_api_table">
								<tr>
									<th><?php _e('Meeting Type: ', 'zoom-video-conference'); ?></th>
									<td><select name="meetingType" id="meetingType">
										<option value="2" <?php echo ($results['type']==2) ? 'selected' : ''; ?>><?php _e('Scheduled Meeting', 'zoom-video-conference'); ?></option>
									</select></td>
								</tr>
								<tr>
									<th><?php _e('Start Date/Time: <br><small style="color:red">(Please specify both fields Date and Time)</small>', 'zoom-video-conference'); ?></th>
									<?php 
									...
									<td>Previous Date:<?php echo ($date) ? $date.'T' : ''; ?><br>
										...
										<input type="text" name="start_date"  id="datepicker1" class="datepicker1" value=""><br>
										Previous Time: <?php echo ($time) ? $time : ''; ?><br>
										<input type="hidden" name="start_time_hidden" value="<?php echo ($time) ? $time : ''; ?>">
										<input type="text" name="start_time"  class="timepicker1" class="timepicker1" value=""></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td><input type="text" name="duration" value="<?php echo ($results['duration']) ? $results['duration'] : ''; ?>"></td>
									</tr>
									<tr>
										<th><?php _e('Enable join before host <br><small>(Only for scheduled and Recurring meetings)</small>', 'zoom-video-conference'); ?></th>
									...
									<tr>
										...
										<td>
											<input type="radio" name="option_participants_video" value="true" <?php echo ($results['option_participants_video']==true) ? 'checked' : ''; ?>> Yes
											<input type="radio" name="option_participants_video" value="false" <?php echo ($results['option_participants_video']==false) ? 'checked' : ''; ?>> No
										</td>

									</tr>
									<tr>
										<th><?php _e('Require meeting password', 'zoom-video-conference'); ?></th>
										<td><input type="checkbox" name="pwd_enabled" class="pwd_enabled" <?php echo ($results['password']) ? 'checked' : ''; ?> >
