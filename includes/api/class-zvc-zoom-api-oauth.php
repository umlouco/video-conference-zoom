...
class ZoomAuth {
	...
	public function make_remote_post_request() {
		...
		if ( is_wp_error( $token_info_response ) ) {
			...
			$zoom_user_info_arr = $zoom_user_info_response['zoom_user_info'];

			$this->oauth_ret['success']              = true;
			$this->oauth_ret['zoom_user_info']       = $zoom_user_info_arr;
			$this->oauth_ret['zoom_user_token_info'] = $token_info_arr;
	...
	public function get_zoom_user_info_with_access_token( $token_type, $access_token ) {
		...
		if ( is_wp_error( $response_with_access_token ) ) {
			...
			$this->oauth_ret['success']        = true;
			$this->oauth_ret['zoom_user_info'] = $body;
	...
	public function refresh_access_token( $refresh_token ) {
		...
		if ( $refresh_action_post_request['response']['code'] === 200 ) {
			...
			$this->oauth_ret['success']              = true;
			$this->oauth_ret['zoom_user_token_info'] = $refreshed_tokens;
	...
	public function deauthorize( $user_id ) {
		...
		$tokens = get_user_meta( $user_id, 'zoom_user_token_info', true );
